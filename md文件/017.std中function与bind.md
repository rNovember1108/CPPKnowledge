...
可将 `std::bind` 函数看作一个通用的函数适配器，它接受一个可调用对象，生成一个新的可调用对象来**适应**原对象的参数列表。`std::bind` 将可调用对象与其参数一起进行绑定，绑定后的结果可以使用 `std::function` 保存。`std::bind` 主要有以下两个作用：
...
3. 绑定一个引用参数
    ...
    int cy = 2, cz = 3;
    auto fn_half = std::bind(my_divide, std::placeholders::_1, std::ref(cy), std::cref(cz));

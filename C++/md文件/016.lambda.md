...
- 值捕获：将变量的副本拷贝到lambda的闭包对象中。修改副本不会影响原始变量；使用=可以将函数作用域的所有变量以值捕获方式传入到表达式中。
...
用-> type可以指定lambda表达式返回值类型；如果不指定返回类型，则编译器会根据代码实现为函数推导一个返回类型；如果没有返回值，则可忽略此部分。
...
c++Lambda表达式是一项重要的语言特性，它使得开发人员可以更方便地创建匿名函数和函数对象。Lambda表达式的语法简洁、灵活，可以捕获外部变量，并且可以作为任何需要函数对象的地方使用。Lambda表达式也存在一些缺点，包括可读性、复杂性和性能问题。在实际开发中，开发人员应该根据具体情况权衡利弊，选择合适的编程风格和实现方式。
